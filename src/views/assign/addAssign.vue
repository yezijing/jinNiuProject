<template>
  <div class="addproject">
        ...
        <fieldset>
          ...
          <div class="boxcontent basicbox">
            <el-form label-width="110px" size="small" ref="basicInfo" :model="basicInfo">
              <el-col :span="12" class="assign-left">
                <el-form-item
                  label="编号："
                  prop="code"
                  :rules="[{ required: true, message: '请输入编号', trigger: 'change' }]"
                >
                  <el-input placeholder="请输入编号" v-model="basicInfo.code"></el-input>
                </el-form-item>
                <el-form-item label="立项时间：" prop="starttime">
                  <el-date-picker v-model="basicInfo.starttime" type="date" placeholder="选择时间"></el-date-picker>
                </el-form-item>
              ...
              <el-col :span="12" class="assign-right">
                <el-form-item label="任务来源：" prop="tasksource">
                  <el-input placeholder="请输入任务来源" v-model="basicInfo.tasksource"></el-input>
                </el-form-item>
        ...
        <el-col :span="12">
          <fieldset>
            ...
            <div class="boxcontent">
              <el-button class="addbtn" size="small" @click="addTask" type="primary">新 增</el-button>
              <el-table
                border
                :data="checkMeetList"
              ...
              >
                ...
                <el-table-column prop="stoptime" label="时间节点"></el-table-column>
                <el-table-column label="操作">
                  <template scope="scope">
                    <el-button class="btn" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                    <el-button class="btn" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
    ...
    <!-- 新增督办事项 -->
    <el-dialog :title="itemTitle" :visible.sync="itemVisible" width="800px" class="task-dialog">
      <el-form
        :model="addItemData"
        size="small"
        label-width="130px"
      ...
      >
        <el-form-item
          label="督办事项："
          prop="remark"
          :rules="[{ required: true, message: '督办事项不能为空', trigger: 'change' }]"
        >
          <el-input
            type="textarea"
            :autosize="{ minRows: 3}"
        ...
        <!-- <div class="taskbox taskbox2">
          <el-form-item label="经办人：" prop="agentname">
            <el-input v-model="addItemData.agentname" :readonly="readOn"></el-input>
          </el-form-item>
          <el-form-item label="联系方式：" prop="phone">
            <el-input v-model="addItemData.phone" :readonly="readOn"></el-input>
          </el-form-item>
        </div>-->
        <el-form-item
          label="截止报送时间："
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="submitItemForm" type="primary">{{changeWord}}</el-button>
      </div>
    </el-dialog>
...
export default {
  props: ["deptlist", "addheight", "leaderUsers"],
  components: {},
  data() {
    return {
      // 报送类型
  ...
  methods: {
    ...
    addSubmit: function() {
      this.$refs.addItemData.validate(valid => {
        if (valid) {
          this.addItemData.status = 0;
          let para = Object.assign({}, this.addItemData);
          this.checkMeetList.push(para);
    ...
    editSubmit() {
      this.$refs.addItemData.validate(valid => {
        if (valid) {
          let para = Object.assign({}, this.addItemData);
          this.checkMeetList.splice(this.tindex, 1, para);
          this.$refs["addItemData"].resetFields();
          this.itemVisible = false;
        } else {
          return false;
    ...
    handleDelete(index, row) {
      this.checkMeetList.splice(index, 1);
      this.$refs["addItemData"].resetFields();
      this.itemVisible = false;
    },
    // 删除数组中某个元素
      ...
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          if (this.leadsData == []) {
            ...
            para.totalstatus = 0;
            para.formstatus = 0;
            para.checkMeetList = this.checkMeetList;
            para.checkRuleList = this.ruleList;
            ...
            addAssign(para, res => {
              if (res.data.code == -2) {
                ...
                if (res.data.code == 0) {
                  let cpara = { cid: res.data.obj };
                  changeCheckFormstate(cpara, data => {
                    synchFormstate({ cid: res.data.obj }, ress => {
                      this.$emit("my-event");
...
.addcontent .box fieldset{width: 100%;}
</style>

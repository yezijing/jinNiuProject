<template>
  <div class="addproject">
          ...
          <fieldset>
            ...
            <div class="boxcontent">
              ...
              >
                ...
                <el-table-column prop="stoptime" :formatter="timestampToTime" label="时间节点"></el-table-column>
    ...
    <div class="goback">
      <el-button size="small" type="warning" @click="goback">取 消</el-button>
      <el-button size="small" @click="submitAll" type="primary">保 存</el-button>
    </div>
    ...
    <el-dialog :title="itemTitle" :visible.sync="itemVisible" width="800px" class="task-dialog">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="submitItemForm" size="small" type="primary">{{changeWord}}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  created() {
    ...
    if (this.fileList == []) {
      ...
      for (var i = 0; i < this.fileList.length; i++) {
        this.filepath.push(this.fileList[i].name);
      }
    }
    this.upfile = upfile;
    this.imgIp = imgIp;
  ...
  methods: {
    ...
    deptChange2() {
      ...
      //     deptname = this.deptlist[i].name;
      //     leadid = this.deptlist[i].deptLeader;
      //     deptphone = this.deptlist[i].deptphone;
      //     break;
      //   }
      // }
    ...
    handleEdit(index, row) {
      this.changeNum = "edit";
      this.changeWord = "保存";
      this.itemTitle = "编辑督办事项";
      this.readOn = false;
      this.itemVisible = true;
      this.addItemData = Object.assign({}, row);
      this.addItemData.stoptime = new Date(this.addItemData.stoptime);
      this.tindex = index;
    },
    // 弹窗-确认提交
    ...
    addSubmit: function() {
      this.$refs.addItemData.validate(valid => {
        if (valid) {
          ...
          this.itemVisible = false;
          console.log(this.checkMeetList);
        } else {
          return false;
        }
      });
    ...
    // 完结
    handleOver(row, id, index) {
      if (row.status == -1) {
        this.$message({
          message: "已完结"
        });
      } else {
        this.$confirm("确定设置为完结吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
          .then(() => {
            ...
            planOver(para, data => {
              planbyId(para, res => {
                this.checkMeetList[index].status = res.data.obj.status;
              });
            });
          })
          .catch(() => {
    ...
    // 滞后
    handleLag(row, id, index) {
      if (row.status == 3) {
        this.$message({
          message: "已设置滞后"
        });
      } else {
        this.$confirm("确定设置为滞后吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
    ...
    submitAll: function() {
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          ...
          var date = new Date(this.basicInfo.starttime);
          var Y = date.getFullYear() + "-";
          var M =
            (date.getMonth() + 1 < 10
              ? "0" + (date.getMonth() + 1)
          ...
          this.basicInfo.starttime = Y + M + D;
          ...
          assignEdit(para, res => {
            changeCheckFormstate({ cid: this.ccid }, data => {
              debugger;
              synchFormstate({ cid: this.ccid }, ress => {
                debugger;
                this.$emit("my-event");

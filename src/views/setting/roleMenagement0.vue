<template>
  <div class="content-wrap setting setting2">
    ...
    <!-- 管理成员 -->
    <!-- <el-dialog title="管理成员" :visible.sync="memberVisible" width="800px" class="memberbox">
      <p class="title">添加新成员</p>
      <div class="mlist">
        <el-checkbox
        ...
        >全选</el-checkbox>
        <div class="mcut"></div>
        <el-checkbox-group v-model="checkedUsers" @change="handleCheckedUserChange">
          <el-checkbox v-for="item in users" :label="item.id" :key="item.id">{{item.name}}</el-checkbox>
        </el-checkbox-group>
      </div>
      <p class="title">已有成员</p>
      <div class="mlist">
        <el-checkbox-group v-model="hasUsers">
          <el-checkbox v-for="item in hasList" :label="item.id" :key="item.id">{{item.name}}</el-checkbox>
        </el-checkbox-group>
      </div>
      <div slot="footer" class="dialog-footer">
    ...
    <!-- 管理成员 -->
    <el-dialog title="成员管理" :visible.sync="memberVisible" width="900px" class="memberbox">
      <div class="addbox">
        <p class="title">添加成员</p>
        <div class="treelist">
          <el-row :gutter="20">
            <el-col :span="10" class="partlist">
              <p class="tt">选择部门</p>
              <el-tree
                :data="organizeList"
                accordion
  ...
  methods: {
    ...
    handleMember(index, row) {
      ...
      memberList(para, data => {
        // this.users = data.data.obj.hasNoRoleUser;
        // for(var j=0;j<this.users.length;j++){
        //   this.defaultKeys.push(this.users[j].id)
        // }
        if (data.data.obj.hasRoleUser == null) {
          data.data.obj.hasRoleUser = [];
        }
        debugger

...
export default {
  ...
  methods: {
    ...
    removeToHaslist() {
      for (var i = 0; i < this.hasUsers.length; i++) {
        ...
        for (var j = 0; j < this.hasList.length; j++) {
          if (cid == this.hasList[j].id) {
            ...
            for(var n=0;n<this.hasArr.length;n++){
              if(this.hasArr[n]==cid){
                this.hasArr.splice(n,1);
                break;
              }
            }
            break;
      ...
      this.hasitem = [];
      let para = { orgid: this.curid, roleid: this.roleid };
      getAllusers(para, data => {
        ...
        var array = this.cities; //当前机构下所有成员列表
        var array2 = this.hasArr; //已有成员id集合
    ...
    addToHaslist() {
      for (var i = 0; i < this.checkedCities.length; i++) {
        ...
        if (ison == true) {
          for (var m = 0; m < this.hasList.length; m++) {
            if (cid == this.hasList[m].id) {
              this.hasList.splice(m, 1);
            }
          }
          this.hasList.push(obj);
          this.hasArr.push(cid);
    ...
    addSubmit: function() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          ...
          checkedKeys.push('main','mainindex');
          new Set(checkedKeys)
    ...
    // 管理成员提交
    memberSubmit() {
      let para = { roleid: this.roleid, userids: this.hasArr };
      para = JSON.stringify(para);
      addMember(para, data => {
        this.$message({
          message: "保存成功",

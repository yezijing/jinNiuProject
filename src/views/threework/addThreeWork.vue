              ...
              <el-form label-width="120px" size="small" ref="basicInfo" :model="basicInfo">
                ...
                <div class="addbox addbox-area">
                  <el-form-item label="项目名称：" prop="projectname">
                    <el-input placeholder="请输入" v-model="basicInfo.projectname"></el-input>
                  </el-form-item>
                </div>
                <div class="addbox addbox-area">
...
import { getAllProject, checkReportRules } from "../../api/projectApi";
import { addKeyItem,changeKeyitemFormstate,synchKeyitemFormstate } from "../../api/threework";
export default {
  ...
  methods: {
    ...
    submitAll: function() {
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          var aids = this.basicInfo.arealeadids;
          // 牵头区领导id集合
          if (aids.length == 0) {
            aids = "";
          } else {
          ...
          var superdept = this.basicInfo.superdept.join(",");
          let para = {
            year: this.basicInfo.yearid,
            projectname: this.basicInfo.projectname,
            timenode: this.basicInfo.timenode,
            superdept: superdept
          };
          para.lederid = aids;
          para.ldeptList = this.leadsData;
          para.dutyDeptList = this.dutysData;
          para.ruleList = this.ruleList;
          para = JSON.stringify(para);
          addKeyItem(para, res => {
            if (res.data.code == "0") {
              changeKeyitemFormstate({ eid: res.data.obj }, data => {
                synchKeyitemFormstate({ eid: res.data.obj }, ress => {
                  this.$emit("my-event");
                  this.$emit("add-event");
                  this.$message({
                    message: "新增成功",

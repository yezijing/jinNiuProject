  ...
  <div class="content-wrap decision threeyear">
        ...
        >
          ...
          <el-table-column prop="edit" label="操作" fixed="right">
            <template scope="scope">
              <el-button
                class="btn"
                @click="handleDetail(scope.$index, scope.row, scope.row.pointid)"
              >详情</el-button>
              <el-button
                class="btn"
                @click="handleEdit(scope.$index, scope.row, scope.row.pointid)"
                v-if="isEditShow"
              >编辑</el-button>
              <el-button
                class="btn"
                @click="handleDelete(scope.$index, scope.row, scope.row.pointid)"
                v-if="isEditShow"
              >删除</el-button>
              <el-button
                class="btn"
    ...
    <div class="import-data-box">
      <import-file
        ...
        :itemShow="itemShow"
        :importFileList="importZhongWorkList"
...
<script>
import addPlan from "./addImportWork"; //新增
import editPlan from "./editImportWork"; //新增
import detailPlan from "./detailImportWork"; //详情
...
import { upfile, imgIp, importZhongWorkList } from "../../api/imgUrl";
import {
  getYear,
  importWorkListData,
  addImportworkReport,
  ...
  threeworkList,
  changeTotalstateImportwork,
  getThreeBypartid,
  repeatReportImportwork,
  getStatusFormImportwork,
  deleteImportWork,
  showImportworkLasttime,
  lasttimeProgressImportwork,
  getAllImportworkList,
  seerList
...
export default {
  ...
  created() {
    ...
    this.importZhongWorkList = importZhongWorkList;
    this.upfile = upfile;
    this.imgIp = imgIp;
    this.hiddenDatas();
    this.browerHeight();
  ...
  methods: {
    ...
    downDatalist() {
      if (this.downType == 1) {
        window.open(this.defaturl + "/report/createPointReport");
      } else {
        window.open(this.defaturl + "/pointExport2Word/export");
      }
    ...
    totalstateChange(scope, row) {
      ...
      changeTotalstateImportwork(para, data => {
        this.getThreePlan();
        this.$message({
          type: "success",
          message: "修改状态成功!"
    ...
    hiddenDatas() {
      let para = { userid: this.loginid, catagory: 2 };
      isHidden(para, data => {
    ...
    handleEvolve(index, row) {
      ...
      // 获取上一次进展情况
      let repa = {};
      repa.pointid = this.pointid;
      repa.userid = this.loginid;
      showImportworkLasttime(
        { userid: this.loginid, pointid: this.pointid },
        data => {
          this.laststate = data.data.obj.question;
        }
      );
      // 基本信息
      this.redetailFuc(this.pointid);
      // 判断是否重复报送
    ...
    // 详情方法
    detailFuc(para) {
      importWorkDetailData(para, data => {
        this.detailData = data.data.obj;
        this.basicInfo = this.detailData;
        ...
        lasttimeProgressImportwork(
          { pointid: this.pointid, userid: this.loginid },
          data => {
    ...
    handleDetail: function(index, row, id) {
      ...
      this.pointid = id;
      let para = { pointid: id };
      getStatusFormImportwork(para, data => {
        let spara = { pointid: id };
        this.detailFuc(spara);
      });
    },
    // 详情进入编辑
    toeditPage() {
      let para = { pointid: this.pointid };
      this.basicInfo = {};
      importWorkDetailData(para, data => {
        this.detailData = data.data.obj;
        this.basicInfo = this.detailData;
        ...
        if (this.basicInfo.yearid == null) {
          ...
          let spara = { yearid: this.basicInfo.yearid };
          importWorkFirstList(spara, data => {
        ...
        if (this.basicInfo.yearid == null || this.basicInfo.bigid == null) {
          ...
          importWorkBigMinList(spara, data => {
    ...
    // 个人中心进入详情
    centerDetail() {
      if (this.$route.query.pointid == undefined) {
        this.pointid = "";
        this.isCenter = 3;
        this.getThreePlan();
      } else {

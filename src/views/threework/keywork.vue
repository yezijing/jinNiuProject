  ...
  <div class="content-wrap">
    <div v-if="allShow">
      <div v-show="isShow">
        ...
        >
          ...
          <el-table-column prop="tripleWorkProgress" label="任务名称">
            <template slot-scope="scope">
              <p>{{scope.row.tripleWorkProgress.workname}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="tripleWorkProgress" label="重要时间节点">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" placement="top-start">
                <div
                  style="max-width:300px;line-height:1.8;"
                  slot="content"
                >{{scope.row.tripleWorkProgress.nodeplan}}</div>
                <p
                  class="title-ellipsis title"
                  @click="handleDetail(scope.$index, scope.row, scope.row.tripleWorkProgress.id)"
                >{{scope.row.tripleWorkProgress.nodeplan}}</p>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="tripleWorkProgress" label="督查单位">
            <template slot-scope="scope">
              <p>{{scope.row.tripleWorkProgress.secondwork}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="workLeaderNames" label="牵头区领导" v-if="ishidden1">
            <template slot-scope="scope">
              <p v-html="namesChange(scope,scope.row.workLeaderNames)"></p>
            </template>
          </el-table-column>
          <el-table-column prop="tripleWorkProgressLeadDeptList" label="牵头单位" v-if="ishidden2">
            <template slot-scope="scope">
              <p
                v-for="(item, index) in scope.row.tripleWorkProgressLeadDeptList"
                :key="index"
              >{{item.deptName}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="tripleWorkProgressDutyDeptList" label="责任单位" v-if="ishidden3">
            <template slot-scope="scope">
              <p
                v-for="(item, index) in scope.row.tripleWorkProgressDutyDeptList"
                :key="index"
              >{{item.deptName}}</p>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="tripleWorkProgress" label="材料报送" width="100px">
            <template slot-scope="scope">
              <div>
                <i
                  class="fa mark"
                  v-if="scope.row.tripleWorkProgress.formstatus==1"
                  style="background:#fbe804;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.tripleWorkProgress.formstatus==2"
                  style="background:#f00;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.tripleWorkProgress.formstatus== -1"
                  style="background:#999;"
                ></i>
                <i class="fa mark" v-else></i>
                <span v-html="formstatusFormat(scope,scope.row.tripleWorkProgress.formstatus)"></span>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="tripleWorkProgress" label="总体进度" width="120px">
            <template slot-scope="scope">
              <div class="flexbox" v-if="isEditShow">
                <i
                  class="fa mark"
                  v-if="scope.row.tripleWorkProgress.totalstatus==3"
                  style="background:#364e98;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.tripleWorkProgress.totalstatus==4"
                  style="background:#7c1d77;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.tripleWorkProgress.totalstatus== -1"
                  style="background:#999;"
                ></i>
                <i class="fa mark" v-else></i>
                <el-select
                  v-model="scope.row.tripleWorkProgress.totalstatus"
              ...
              <div v-else>
                ...
                ></i>
                <i class="fa mark" v-else></i>
                <span v-html="totalstatusFormat(scope,scope.row.tripleWorkProgress.totalstatus)"></span>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="edit" label="操作" fixed="right">
            <template scope="scope">
              ...
              <el-button
                class="btn"
                @click="handleEdit(scope.$index, scope.row, scope.row.tripleWorkProgress.id)"
                v-if="isEditShow"
              >编辑</el-button>
              <el-button
                class="btn"
                @click="handleDelete(scope.$index, scope.row, scope.row.tripleWorkProgress.id)"
                v-if="isEditShow"
              >删除</el-button>
              <el-button
                class="btn"
    ...
    <div class="import-data-box">
      <import-file
        ...
        :importFileList="importKeyworkList"
...
<script>
import addKeywork from "./addKeywork"; //新增
import editKeywork from "./editKeywork"; //新增
import detailKeywork from "./detailKeywork"; //详情
...
import {
  ...
  repeatReportKeywork,
  addKeyworkReport,
  changeTotalstateKeywork,
  deleteKeywork,
  getAllKeywork,
  showKeyworkLastReport,
  lasttimeKeyworkProgress
} from "../../api/threework";
import { defaturl } from "../../api/apiUrl";
import { upfile, imgIp, importKeyworkList } from "../../api/imgUrl";
export default {
  name: "threePlan",
  components: {
    addKeywork,
  ...
  created() {
    ...
    this.importKeyworkList = importKeyworkList;
    this.hiddenDatas();
    this.browerHeight();
    this.isshowCheck();
    this.getDeptlist();
  ...
  methods: {
    ...
    downDatalist() {
      if (this.downType == 1) {
        window.open(
          this.defaturl + "/tripleWorkProgress/tripleWorkExportExcel"
        );
      } else {
        window.open(this.defaturl + "/tripleWorkProgress/tripleWorkExport");
      }
    ...
    merage() {
      ...
      for (var i = 0; i < this.tableData.length; i += 1) {
        ...
        if (i == 0) {
          ...
          if (
            this.tableData[i].tripleWorkProgress.workname ==
            this.tableData[i - 1].tripleWorkProgress.workname
          ) {
          ...
          // 第2列
          if (
            this.tableData[i].tripleWorkProgress.nodeplan ==
            this.tableData[i - 1].tripleWorkProgress.nodeplan
          ) {
    ...
    // 下载模板
    downMoudle() {
      window.open(this.defaturl + "/templateDownload/tripleWorkImportTemplate");
    },
    // 序号
    ...
    totalstateChange(scope, row) {
      let para = {
        totalstatus: row.tripleWorkProgress.totalstatus,
        workid: row.tripleWorkProgress.id
      };
      changeTotalstateKeywork(para, data => {
        this.getThreePlan();
        this.$message({
          type: "success",
          message: "修改状态成功!"
    ...
    redetailFuc(id) {
      keyWorkDetail({ workid: id }, data => {
        ...
        var str = this.detailData.tripleWorkProgress.dutyleadname;
        if (str == null) {
    ...
    handleEvolve(index, row) {
      ...
      this.workid = row.tripleWorkProgress.id;
      // 基本信息
      this.redetailFuc(this.workid);
      // 上一次进展情况
      lasttimeKeyworkProgress({ workid: this.workid }, data => {
        if (data.data.obj == null) {
          this.laststate = "";
        } else {
    ...
    // 详情方法
    detailFuc(para) {
      keyWorkDetail(para, data => {
        this.detailData = data.data.obj;
        this.basicInfo = this.detailData.tripleWorkProgress;
        this.basicInfo.yearname = this.detailData.tripleWorkProgress.year;
        var str = this.detailData.tripleWorkProgress.dutyleadname;
        if (str == null) {
        ...
        this.leadsData = this.detailData.tripleWorkProgressLeadDeptList;
        this.dutysData = this.detailData.tripleWorkProgressDutyDeptList;
        this.ruleList = this.detailData.tripleWorkProgressRuleList;
    ...
    handleDetail: function(index, row, id) {
      if (row.tripleWorkProgress.totalstatus == -1) {
        this.noEdit = false;
      } else {
        this.noEdit = true;
      }
    ...
    editFuc(para) {
      keyWorkDetail(para, data => {
        ...
        this.detailData = data.data.obj;
        this.basicInfo = this.detailData.tripleWorkProgress;
        // this.basicInfo.secondwork = this.detailData.tripleWorkProgress.secondwork.split(',');
        // 牵头区领导
        var aids = this.detailData.tripleWorkProgress.dutyleaderid;
        var aidarr = [];
        if (aids == null || aids == "") {
          aidarr = [];
          ...
          //     : aids;
          aidarr = aids.split(",");
          this.basicInfo.arealeadids = aidarr;
        }
        ...
        this.newReportList = this.detailData.reportList;
        if (this.newReportList.length == 0) {
        } else {
          for (var i = 0; i < this.newReportList.length; i++) {
            this.newReportList[i].newFiles = [];
            var arr = this.newReportList[i].filepath;

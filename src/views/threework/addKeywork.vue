...
import { addKeyWork, oneLevelList, keyLevelList, changeKeyworkFormstate, synchKeyworkFormstate } from "../../api/threework";
export default {
  ...
  methods: {
    ...
    addRuleSubmit() {
      this.$refs.addRuleForm.validate(valid => {
        if (valid) {
          ...
          para.widList = this.checkedItems;
          this.cycleNum = this.addRuleForm.cycletype;
          if (this.cycleNum == 1) {
    ...
    // 新增重点工作
    addBigSubmit() {
      this.biglist.push({ workname: this.bitemname });
    ...
    // 新增一级任务
    addSubterm() {
      this.minlist.push({ nodeplan: this.sitemname });
    ...
    submitAll: function() {
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          var decisionUp = this.basicInfo;
          decisionUp.secondwork = this.basicInfo.secondwork.join(',')
          ...
          decisionUp.createuser = this.loginid;
          decisionUp.year = year;
          let opara = { tripleWorkProgress: decisionUp };
          opara.tripleWorkProgressLeadDeptList = this.leadsData;
          opara.tripleWorkProgressDutyDeptList = this.dutysData;
          opara.tripleWorkProgressRuleList = this.ruleList;
          opara = JSON.stringify(opara);

          addKeyWork(opara, res => {
            if (res.data.code == -2) {
              this.$message({
                message: "新增失败！",
                type: "error"
              ...
              if (res.data.code == 0) {
                let cpara = { workid: res.data.obj };
                changeKeyworkFormstate(cpara, data => {
                  synchKeyworkFormstate({ workid: res.data.obj }, ress => {
                    this.$emit("my-event");
                    this.$emit("add-event");
                    this.$message({
                      message: "新增成功",

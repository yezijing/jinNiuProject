  ...
  <div class="content-wrap decision threeyear">
    <div v-if="allShow">
      <div v-show="isShow">
        ...
        <el-row>
          <el-col :span="24" class="toolbar toolbar2">
            <el-form :inline="true" :model="filters" ref="filters" size="small">
              ...
              <el-form-item label="牵头区领导：" prop="arealeaderid" class="ld" @change="selectChange">
                <el-select v-model="filters.arealeaderid" clearable filterable placeholder="全部">
                  <el-option
        ...
        >
          <el-table-column prop="bigname" label="工作要求">
          ...
          <el-table-column prop="workname" width="150px" label="推进内容（重点）" v-if="ishidden8">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" placement="top-start">
          ...
          <el-table-column prop="nodeplan" label="工作计划" v-if="ishidden5">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" placement="top-start">
...
export default {
  ...
  methods: {
    ...
    downDatalist() {
      if (this.downType == 1) {
        window.open(this.defaturl + "/report/createIndicateReport");
      } else {
        window.open(this.defaturl + "/ledIndicateExport2Word/export");
      }
    ...
    merage() {
      ...
      for (let i = 0; i < this.tableData.length; i += 1) {
        ...
        if (i === 0) {
          ...
          // 第2列
          if (
            this.tableData[i].minname == this.tableData[i - 1].minname &&
            this.tableData[i].bigname == this.tableData[i - 1].bigname
          ) {
          ...
          // 第3列
          if (
            this.tableData[i].partname == this.tableData[i - 1].partname &&
            this.tableData[i].minname == this.tableData[i - 1].minname &&
            this.tableData[i].bigname == this.tableData[i - 1].bigname
          ) {
    ...
    // 进展基本信息
    redetailFuc(id) {
      threePlanDetail({ workid: id, typemodel: 2 }, data => {
    ...
    handleEvolve(index, row) {
      ...
      repa.typemodel = 2;
      showThreeplanLasttime(
        { userid: this.loginid, workid: this.workid, typemodel: 2 },
        data => {
    ...
    detailFuc(para) {
      threePlanDetail(para, data => {
        ...
        // this.basicInfo.leaders = this.detailData.areaLeaderNames;
        ...
        lasttimeProgress(
          { typemodel: 2, workid: this.workid, userid: this.loginid },
          data => {
    ...
    handleDetail: function(index, row, id) {
      ...
      this.noEdit = true;
      this.workid = id;
      let para = { workid: id, typemodel: 2 };
    ...
    centerDetail() {
      if (this.$route.query.workid == undefined) {
        ...
        if (this.$route.query.sortnum == 0) {
          ...
          this.detailShow = true;
          this.noEdit = true;
          let para = { workid: this.workid, typemodel: 2 };
          getStatusForm(para, data => {});
          this.detailFuc(para);
        } else {
          return false;
    ...
    editFuc(para) {
      threePlanDetail(para, data => {
        ...
        if (this.basicInfo.yearid == null) {
          ...
          let spara = { yearid: this.basicInfo.yearid, typemodel: 2 };

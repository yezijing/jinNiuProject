<template>
  <div class="addplan">
    ...
    <div class="addcontent" :style="addheight">
      ...
      <el-row class="box" :gutter="30">
        <el-col :span="12">
          <fieldset>
            <legend>基本信息</legend>
            <div class="boxcontent boxcontent0">
              <el-form label-width="120px" size="small" ref="basicInfo" :model="basicInfo">
                <div class="addbox">
                  <el-form-item
                    label="年度："
                    prop="yearid"
                  ...
                  >
                    <el-select v-model="basicInfo.yearid" filterable clearable placeholder="请选择">
                      <el-option
                        v-for="item in years"
                        :key="item.id"
                        :label="item.year"
                  ...
                  <div class="jiabtn">
                    ...
                    <div class="awrap" v-if="yearShow">
                      <p>新增年度</p>
                      <el-input placeholder="请输入年度名称" v-model="yearname"></el-input>
                      <div class="buttons">
                ...
                <div class="addbox">
                  <el-form-item
                    :label="typemodel==1?'一级工作：':'工作要求：'"
                    prop="bigid"
                    :rules="[{ required: true, message: '请选择', trigger: 'change' }]"
                  >
                    <el-select
                      v-model="basicInfo.bigid"
                      filterable
                      clearable
                      placeholder="请选择"
                      @focus="getbigterm()"
                  ...
                  <div class="jiabtn">
                    <i class="icon el-icon-circle-plus" @click="bitemShow = true"></i>
                    <div class="awrap" v-if="bitemShow">
                      <p>新增</p>
                      <el-input placeholder="请输入" v-model="bitemname"></el-input>
                      <div class="buttons">
                        <el-button @click="bitemShow = false">取消</el-button>
                        <el-button @click="addBigSubmit()" type="primary">确定</el-button>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="addbox">
                  <el-form-item label="二级工作：" prop="minid">
                    <el-select
                      v-model="basicInfo.minid"
                      filterable
                      clearable
                      placeholder="请选择"
                      @focus="getBiglist()"
                  ...
                  <div class="jiabtn">
                    <i class="icon el-icon-circle-plus" @click="sitemShow = true"></i>
                    <div class="awrap" v-if="sitemShow">
                      <p>新增</p>
                      <el-input placeholder="请输入" v-model="sitemname"></el-input>
                      <div class="buttons">
                        <el-button @click="sitemShow = false">取消</el-button>
                        <el-button @click="addSubterm" type="primary">确定</el-button>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="addbox">
                  <el-form-item label="三级工作：" prop="partid">
                    <el-select
                      v-model="basicInfo.partid"
                      clearable
                      placeholder="请选择"
                      @focus="getThreeworkList()"
                    >
                      <el-option
                        v-for="item in threeitemList"
                        :key="item.id"
                        :label="item.partname"
                        :value="item.id"
                      ></el-option>
                  ...
                  <div class="jiabtn">
                    <i class="icon el-icon-circle-plus" @click="threeShow = true"></i>
                    <div class="awrap" v-if="threeShow">
                      <p>新增</p>
                      <el-input placeholder="请输入" v-model="threename"></el-input>
                      <div class="buttons">
                        <el-button @click="threeShow = false">取消</el-button>
                        <el-button @click="newThree" type="primary">确定</el-button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="addbox">
                  <el-form-item label="牵头区领导：" prop="aleaders" class="qt">
                    <el-select
                      v-model="basicInfo.aleaders"
                      multiple
                      clearable
                      collapse-tags
                      placeholder="请选择"
                ...
                <div class="addbox addbox-area">
                  <el-form-item :label="typemodel==1?'工作计划：':'推进内容(重点)：'" prop="workname">
                    <el-input
                      type="textarea"
                      placeholder="请输入"
                      rows="3"
                ...
                <div class="addbox addbox-area">
                  <el-form-item :label="typemodel==1?'节点安排：':'任务计划：'" prop="nodeplan">
                    <el-input
                      type="textarea"
                      placeholder="请输入"
                      rows="3"
    ...
    <div class="goback">
      ...
      <el-button @click="submitAll" size="small" type="primary">新 增</el-button>
    </div>
...
import {
  ...
  threeworkList,
  addThreework,
  changeCheckFormstate,
  synchFormstate,
  getAllPlanlist,
  checkReportRules
} from "../../api/decisionApi";
export default {
  props: ["leaderlist", "deptlist", "addheight", "typemodel","yearid"],
  components: {},
  data() {
    return {
      checkAll: false,
  ...
  created() {
    this.getyear();
    this.basicInfo = {yearid:this.yearid}
  },
  methods: {
    // 周转换
    ...
    getbigterm() {
      if (this.basicInfo.yearid == null) {
        ...
        let para = { yearid: this.basicInfo.yearid, typemodel: this.typemodel };
        bigTermList(para, data => {
    ...
    addBigSubmit() {
      if (this.basicInfo.yearid == null) {
        ...
        let para = {
          ...
          bigname: this.bitemname,
          typemodel: this.typemodel
        };
        addBigterm(para, data => {
    ...
    addSubterm() {
      if (this.basicInfo.yearid == null || this.basicInfo.bigid == null) {
        ...
        let para = {
          bigid: this.basicInfo.bigid,
          yearid: this.basicInfo.yearid,
          minname: this.sitemname,
          typemodel: this.typemodel
        };
        addMinterm(para, data => {
          this.sitemShow = false;
          this.$message({
            message: "新增成功",
            type: "success"
          });
    ...
    newThree() {
      if (
        ...
        let para = {
          ...
          yearid: this.basicInfo.yearid,
          minid: this.basicInfo.minid,
          partname: this.threename,
          typemodel: this.typemodel
        };
        addThreework(para, data => {
          this.threeShow = false;
          this.$message({
            message: "新增成功",
            type: "success"
          });
          this.getThreeworkList();
          this.threename = "";
        });
      }
    },
    // 获取督查单位列表
    getSeerList() {
      // let para = {typemodel:this.typemodel};
      seerList("", data => {
      ...
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          if (this.leadsData == []) {
            ...
            var aids = this.basicInfo.aleaders;
            // 牵头区领导id集合
            if (aids.length == 0) {
              aids = "";
            } else {
            ...
            para.arealeadids = aids;
            para.leaderList = this.leadsData;
            para.dutyList = this.dutysData;
            para.ruleList = this.ruleList;
            para.typemodel = this.typemodel;
            para = JSON.stringify(para);
            addThreePlan(para, res => {
              if (res.data.code == "0") {
                let cpara = { workid: res.data.obj, typemodel: this.typemodel };
                changeCheckFormstate(cpara, data => {
                  synchFormstate(
                    { workid: res.data.obj, typemodel: this.typemodel },
                    ress => {
                      this.$emit("my-event");
                      this.$emit("add-event");
                      this.$message({

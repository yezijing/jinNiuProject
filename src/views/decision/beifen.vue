<template>
  <div class="content-wrap work-sharing">
    <div v-show="isShow">
      <!--工具条-->
      <el-row>
        <el-col :span="24" class="toolbar toolbar2">
          <el-form :inline="true" :model="filters" ref="filters" size="small">
            ...
            <el-form-item label="状态：" prop="status" class="zt" v-if="toolShow">
              <el-select v-model="filters.status" clearable placeholder="全部" @change="selectChange">
                <el-option
            ...
            <div>
              ...
              <el-tooltip effect="dark" placement="top-start">
                <div style="max-width:300px;line-height:2;" slot="content">
                  <p v-for="(item, index) in scope.row.progressList" :key="index">{{item.question}}</p>
                </div>
                <div>
                  <p
    ...
    <!-- 进展填报 -->
    <!-- <el-dialog title="新增进展填报" :visible.sync="evolveVisible" width="700px">
      <el-form
        :model="evolveData"
        ref="evolveData"
        size="small"
      ...
      >
        <el-form-item label="进展情况：" prop="question">
          <el-input type="textarea" :autosize="{ minRows: 3}" v-model="evolveData.question"></el-input>
        </el-form-item>
        <el-form-item label="存在问题：" prop="wenti">
          <el-input type="textarea" :autosize="{ minRows: 3}" v-model="evolveData.wenti"></el-input>
        </el-form-item>
        <el-form-item label="下步工作计划：" prop="measure">
          <el-input type="textarea" :autosize="{ minRows: 3}" v-model="evolveData.measure"></el-input>
        </el-form-item>
        <div class="dia-ww" style="display:flex;">
          <el-form-item label="经办人：" prop="reportuser">
            <el-input v-model="evolveData.reportuser" placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item label="联系方式：" prop="phone">
            <el-input v-model="evolveData.phone" placeholder="请输入"></el-input>
          </el-form-item>
        </div>
        <el-form-item
          label="自我评价："
        ...
        >
          <el-radio-group v-model="evolveData.remark">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="submitEvolveForm" size="small" type="primary">新 增</el-button>
      </div>
    </el-dialog> -->
    ...
    handleEvolve(index, row) {
      ...
      repeatReport(repa, data => {
        if (data.data.obj == true) {
          ...
          this.reportVisible = true;
          // 是否有保存内容
          let repara = { userid: this.loginid, termid: this.workid, modeltype: 5 };

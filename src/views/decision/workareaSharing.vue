    ...
    <div v-if="allShow">
      <div v-show="isShow">
        ...
        >
          ...
          <el-table-column prop="decisionIArea" label="协调解决事项">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" placement="top-start">
                <div
                  style="max-width:300px;line-height:1.8;"
                  slot="content"
                >{{scope.row.decisionIArea.nodeplan}}</div>
                <p
                  class="title-ellipsis title"
                  @click="handleDetail(scope.$index, scope.row, scope.row.decisionIArea.id)"
                >{{scope.row.decisionIArea.nodeplan}}</p>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="decisionIArea" label="市上总体安排" v-if="ishidden5">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" placement="top-start">
                <div
                  style="max-width:300px;line-height:1.8;"
                  slot="content"
                >{{scope.row.decisionIArea.workname}}</div>
                <p class="title-ellipsis">{{scope.row.decisionIArea.workname}}</p>
              </el-tooltip>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="decisionIAreaLeadDeptList" label="牵头单位" v-if="ishidden2">
            <template slot-scope="scope">
              <p
                v-for="(item, index) in scope.row.decisionIAreaLeadDeptList"
                :key="index"
              >{{item.deptName}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="decisionIAreaDutyDeptList" label="责任单位" v-if="ishidden3">
            <template slot-scope="scope">
              <p
                v-for="(item, index) in scope.row.decisionIAreaDutyDeptList"
                :key="index"
              >{{item.deptName}}</p>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="decisionIArea" label="材料报送" width="100px">
            <template slot-scope="scope">
              <div>
                <i
                  class="fa mark"
                  v-if="scope.row.decisionIArea.formstatus==1"
                  style="background:#fbe804;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.decisionIArea.formstatus==2"
                  style="background:#f00;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.decisionIArea.formstatus== -1"
                  style="background:#999;"
                ></i>
                <i class="fa mark" v-else></i>
                <span v-html="formstatusFormat(scope,scope.row.decisionIArea.formstatus)"></span>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="decisionIArea" label="总体进度" width="120px">
            <template slot-scope="scope">
              <div class="flexbox" v-if="isEditShow">
                <i
                  class="fa mark"
                  v-if="scope.row.decisionIArea.totalstatus==3"
                  style="background:#364e98;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.decisionIArea.totalstatus==4"
                  style="background:#7c1d77;"
                ></i>
                <i
                  class="fa mark"
                  v-else-if="scope.row.decisionIArea.totalstatus== -1"
                  style="background:#999;"
                ></i>
                <i class="fa mark" v-else></i>
                <el-select
                  v-model="scope.row.decisionIArea.totalstatus"
              ...
              <div v-else>
                ...
                ></i>
                <i class="fa mark" v-else></i>
                <span v-html="totalstatusFormat(scope,scope.row.decisionIArea.totalstatus)"></span>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="edit" label="操作" fixed="right">
            <template scope="scope">
              ...
              <el-button
                class="btn"
                @click="handleEdit(scope.$index, scope.row, scope.row.decisionIArea.id)"
                v-if="isEditShow"
              >编辑</el-button>
              <el-button
                class="btn"
                @click="handleDelete(scope.$index, scope.row, scope.row.decisionIArea.id)"
                v-if="isEditShow"
              >删除</el-button>
              <el-button
                class="btn"
...
import { upfile, imgIp, importIAreaWorkList } from "../../api/imgUrl";
export default {
  name: "threePlan",
  components: {
    addWorksharing,
  ...
  created() {
    ...
    this.importIAreaWorkList = importIAreaWorkList;
    this.hiddenDatas();
    this.browerHeight();
    this.isshowCheck();
    this.getDeptlist();
  ...
  methods: {
    ...
    totalstateChange(scope, row) {
      let para = {
        totalstatus: row.decisionIArea.totalstatus,
        workid: row.decisionIArea.id
      };
      changeTotalstate2(para, data => {
        this.getThreePlan();
        this.$message({
          type: "success",
          message: "修改状态成功!"
    ...
    handleEvolve(index, row) {
      ...
      this.fileArr = [];
      this.laststate = "";
      this.workid = row.decisionIArea.id;
      // 获取上一次进展情况
    ...
    // 详情方法
    detailFuc(para) {
      threePlanDetail2(para, data => {
        this.detailData = data.data.obj;
        this.basicInfo = this.detailData.decisionIArea;
        this.basicInfo.yearname = this.detailData.decisionIArea.year;
        this.basicInfo.leaders = this.detailData.decisionIArea.arealeadnames;
        this.leadsData = this.detailData.decisionIAreaLeadDeptList;
        this.dutysData = this.detailData.decisionIAreaDutyDeptList;
        this.ruleList = this.detailData.decisionIAreaRuleList;
    ...
    handleDetail: function(index, row, id) {
      if (row.decisionIArea.totalstatus == -1) {
        this.noEdit = false;
      } else {
        this.noEdit = true;
      }
    ...
    editFuc(para) {
      threePlanDetail2(para, data => {
        ...
        this.basicInfo = this.detailData.decisionIArea;
        // 牵头区领导
        var aids = this.detailData.decisionIArea.arealeadids;
        var aidarr = [];
        if (aids == null || aids == "") {
          aidarr = [];

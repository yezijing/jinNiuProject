        ...
        <el-col :span="12">
          <fieldset>
            ...
            <div class="boxcontent boxcontent0">
              <el-form label-width="100px" size="small" ref="basicInfo" :model="basicInfo">
                <div class="addbox">
                  <el-form-item
                    label="年度："
                    prop="yearid"
...
import { addThreePlan, addIAreaWork } from "../../api/workApi";
export default {
  props: ["leaderlist", "deptlist", "addheight", "typemodel"],
  components: {},
  data() {
    return {
      ...
      this.$refs.basicInfo.validate(valid => {
        if (valid) {
          if (this.leadsData == []) {
            ...
            if (decisionUp.arealeadids.length == 0) {
              decisionUp.arealeadids = "";
            } else {
              if (decisionUp.arealeadids.length == 1) {
                decisionUp.arealeadids = decisionUp.arealeadids[0] + ",";
              } else {
                decisionUp.arealeadids = decisionUp.arealeadids.join(",");
              }
            }
            decisionUp.createuser = this.userid;

            // 向上分工
            if (this.typemodel == 1) {
              let para = { decisionUp: decisionUp };
              para.decisionUpLeadDeptList = this.leadsData;
              para.decisionUpDutyDeptList = this.dutysData;
              para.decisionUpRuleList = this.ruleList;
              ...
              // 我区协调
              let opara = { decisionIArea: decisionUp };
              opara.decisionIAreaLeadDeptList = this.leadsData;
              opara.decisionIAreaDutyDeptList = this.dutysData;
              opara.decisionIAreaRuleList = this.ruleList;
              opara = JSON.stringify(opara);
              addIAreaWork(opara, res => {
                this.$message({
                  message: "新增成功",
                  type: "success"
                });
